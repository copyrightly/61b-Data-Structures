...
public class ListSorts {
    ...
    public static LinkedQueue makeQueueOfQueues(LinkedQueue q) {
        ...
        try {
            if (q.size() == 0) {
                ...
                while (q.size() > 0) {
                    LinkedQueue curQ = new LinkedQueue();
                    curQ.enqueue(q.dequeue());
                    qOfqs.enqueue(curQ);
                }
                return qOfqs;
            }
    ...
    public static void partition(LinkedQueue qIn, Comparable pivot,
        ...
        try {
            ...
            System.out.println("ERROR PARTITION");
        }
    }
    ...
    public static void mergeSort(LinkedQueue q) {
        // Your solution here.
        try {
            LinkedQueue qOfqs = makeQueueOfQueues(q);
            while (qOfqs.size() > 1) {
                qOfqs.enqueue(mergeSortedQueues((LinkedQueue) qOfqs.dequeue(), (LinkedQueue) qOfqs.dequeue()));
            }
            if (qOfqs.size() == 1) {
                q.append((LinkedQueue) qOfqs.dequeue());
            }
        }
        catch (QueueEmptyException e) {
